...
public class Graph {
		...
		while (!q.isEmpty()) {
			...
			for (Edge e : current.adjacent) {
				...
				if (neighberNode.weightedCostFromStartVertex == INFINITY) {
					...
					// as the same.
					neighberNode.weightedCostFromStartVertex = current.weightedCostFromStartVertex
							+ 1;
	...
	public void dijkstra(String startName) {
		...
		toVisit.add(new Path(start, 0));
		while (!toVisit.isEmpty()) {
			// dequeues best path
			Path current = toVisit.remove();
			Vertex currentVertex = current.dest;
			currentVertex.scratch = 1;
			for (Edge e : currentVertex.adjacent) {
				Vertex dest = e.dest;
				if ((current.weightedCostOfPath + e.cost) < dest.weightedCostFromStartVertex) {
					dest.weightedCostFromStartVertex = currentVertex.weightedCostFromStartVertex
							+ e.cost;
					dest.prev = currentVertex;
					toVisit.add(new Path(dest, dest.weightedCostFromStartVertex));
				}
			}
		}
	 ...
	 * method is called to collect statistics about each Vertex with the goal of
	 * ranking nodes based on centrality.
	 * 
	 * <br>
	 * <br>
	 * After this method is done, each vertex shall store: <br>
	 * 1. The total number of vertices (not including itself) it is connected to. In
	 * other words, for a given vertex, the number of other vertices for which a
	 * path exists. <br>
	...
	public void findAllPaths(boolean weighted) {
		longest = new Path(new Vertex("temp"), -1);
		for (String s : vertices.keySet()) {
			Vertex curr = vertices.get(s);
			curr.clearPathInfo();
			currentStartVertexName = s;
			if (weighted) {
			...
			for (String c : vertices.keySet()) {
				if (c != s) {
					Vertex connection = vertices.get(c);
					// checks for connection
					if (connection.prev != null) {
						Path connectedPath = getPath(c);
						curr.totalUnweightedPathLength += connection.numEdgesFromStartVertex;
						curr.totalWeightedPathLength += connection.weightedCostFromStartVertex;
						if (longest.weightedCostOfPath < connectedPath.weightedCostOfPath) {
							longest = connectedPath;
						}
	...
	/*
	 * Helper to get path from current start vertex to dest vertex. pre: dest !=
	 * null pre: dest refers to a vertex in this Graph pre: A path exists from the
	 * current start vertex to dest. post: return a Path object with the Path from
	 * the current start vertex to the Vertex specified by dest.
	 */
	 ...
	 * one or more other vertices. Vertices are ordered by centrality. For this
	 * method the most central nodes is the one connected (path exists) to the
	 * largest number of other vertices. Ties based on number of vertices connected
	 * (expected to occur in many graphs) are broken based on the average cost
	 * (weight) of the paths. The lower the average cost, the more central a vertex.
	 * <br>
	...
	public TreeSet<AllPathsInfo> getAllPaths() {
		...
		for (Vertex v : vertices.values()) {
			if (v.numVertexConnected > 0) {
				AllPathsInfo temp = new AllPathsInfo(v.name, v.numVertexConnected,
	 ...
	 * List is returned. <br>
	 * pre: destName != null, containsVertex(destName) == true, the starting vertex
	 * has been set by calling findUnweigthedShortesPath or dijkstra.
	 * 
	 ...
	 *         to destName in which case an empty list is returned.
	 */
	...
	private static class Vertex {
		 ...
		 * totalUnweightedPathLength, and totalWeightedPathLength, are cumulative sum
		 * variables that are to be updated by the findAllPaths in the Graph class.
		 */
		...
		public String toString() {
			return "{" + name + ", " + ", connected to: " + numVertexConnected + "adjacent: "
					+ adjacent + "}";
		}
