...
/**
 *
 * Ranks football teams based on statistics generated from a graph formed by
 * those teams. An edge exists between team A and team B if team A beat team B.
 * The weight of the edge is proportional to the inverse of the margin of
 * victory.
 *
 * The number of other team is connected to AND more importantly the average
 * weight of the paths from a team to the other teams are used to determine the
 * best team.
 *
 */
public class FootballRanker {
	...
	private static final int ROUND_INT = 10;

	private static final double ROUND_DOUBLE = 10.0;
	 ...
	 * pre: gameResults != null, actualRanks != null, files with the given names are
	...
	private double printRootMeanSquareError(List<String> humanRanks, TreeSet<AllPathsInfo> paths,
		...
		// hash all the graph for easy search
		HashMap<String, Integer> graphRanks = new HashMap<>();
		int index = -1;
		for (AllPathsInfo curr : paths) {
			index++;
			graphRanks.put(curr.getName(), index);
		}
		double result = 0.0;
		for (int ranks = 0; ranks < humanRanks.size(); ranks++) {
			int diff = 0;
			if (showResults) {
				System.out.print(String.format("%-" + PADDING + "s - actual rank: %d ",
						humanRanks.get(ranks), (ranks + 1)));
			}
			if (graphRanks.containsKey(humanRanks.get(ranks))) {
				int graphRank = graphRanks.get(humanRanks.get(ranks));
				diff = ranks - graphRank;
				if (showResults) {
					System.out.println("predicted rank: " + (graphRank + 1));
				}
			} else {
				diff = ranks - (paths.size() + 1);
				if (showResults) {
					System.out.println("predicted rank: " + (paths.size() + 1));
				}
			}
			// summation of the squares
			result += diff * diff;
		}
		result /= (humanRanks.size() * 1.0);
		result = Math.sqrt(result);
		if (showResults) {
			System.out.println("Root Mean Square Error: " + ourFormatter.format(result));
		}
		return Math.round(result * ROUND_INT) / ROUND_DOUBLE;
	}
	...
	private Graph buildGraph(String gameResults) {
		...
		try {
			...
			while (sc.hasNext()) {
				...
				int scoreDiff = Integer.parseInt(line[HOME_SCORE])
						- Integer.parseInt(line[AWAY_SCORE]);
				if (scoreDiff != 0) {
